...
class InputWidget(object):
    ...
    def on_value_change(self, change):
        self.data_button.stat_vars[change["owner"].description] = [
            self.options_dict[x] for x in change["new"]
        ]
...
class DataCommonsChoices(object):
    def __init__(self, glue_data_object=None):
        ...
        self.accordion = widgets.Accordion(
            children=[x.input_widget for x in self.input_widgets]
        )
    ...
    def update_data_collection(self, new_data, glue_data_object=None):
        ...
        temp_data = (
            glue_data_object.to_dataframe()
            .set_index("place")
            .drop(["Pixel Axis 0 [x]"], axis=1)
        )
        cols_to_use = new_data.columns.difference(temp_data.columns)
        new_data = temp_data.merge(
            new_data[cols_to_use], left_index=True, right_index=True
        )
    ...
    def get_input_widgets(self):
        household_income_options = {
            "Count of Household: 10,000 USD or Less": "Count_Household_IncomeOfUpto10000USDollar",
        ...
        health_options = {
            "Prevalence: Sleep Less Than 7 Hours": "Percent_Person_SleepLessThan7Hours",
        ...
        environment_options = {
            "Lifetime Air Toxins Cancer Risk": "AirPollutant_Cancer_Risk",
        ...
        return [
            InputWidget(household_income_options, "Income", self.data_button),
